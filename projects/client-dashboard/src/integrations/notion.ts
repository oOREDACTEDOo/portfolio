...
export class NotionIntegration {
   ...
   * Assumes your QSP database has: Title, Status, Due Date, Priority, Client columns
   */
    ...
    for (const client of clients) {
      if (client.notionDatabaseId) {
        try {
          ...
          // Override client name for client-specific databases
  ...
  private parseNotionPage(page: any): NotionTask | null {
    ...
    // Get title (could be named "Name", "Title", or "Task")
    ...
    if (clientProp?.select?.name) {
      ...
      // If client is a relation, we'd need to fetch the related page
      // For now, try to match from title
  ...
  /**
   * Search for tasks mentioning a specific client
   */
  ...
  async createTask(task: {
    ...
    notes?: string;
    source?: string; // e.g., "Fathom meeting: Client Call 2024-01-15"
    ...
    // Add notes/source as description
