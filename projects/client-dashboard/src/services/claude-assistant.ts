...
import { DashboardData, Client, CalendarEvent, NotionTask, FathomMeeting, RankingData } from '../types';
...
`;
    ...
    // Add all tasks for detailed queries
    systemPrompt += `\n### All Active Tasks\n`;
...
4. Recommendations for getting back on track if needed`,
   ...
   */
  async extractActionItems(meetingTitle: string, context: AssistantContext): Promise<{
    meetingTitle: string;
    meetingDate: string;
    client: string | null;
    actionItems: Array<{
